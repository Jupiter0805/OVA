// Regenerates Chapter 1's content in Supabase: deletes the previous
// lessons/pretest/posttest for chapter 1 and inserts the new 8-lesson
// version. Uses the service_role key, so it must only ever be run
// with `node scripts/insertChapter1.js` — never imported from src/.
...
async function main() {
  console.log('🔎 Buscando Capítulo 1...');
  ...
  if (chapterError || !chapter) {
    console.error('❌ No se encontró el Capítulo 1:', chapterError?.message);
    process.exit(1);
  }
  console.log(`   Capítulo 1 = "${chapter.title}" (${chapter.id})\n`);

  const chapterId = chapter.id;

  // --- Clean slate: remove the previous lessons/tests/questions for this chapter ---
  console.log('🧹 Limpiando contenido anterior del Capítulo 1...');
  ...
  // --- Insert lessons ---
  console.log('📚 Insertando 8 lecciones...');
  ...
  // --- Reset stale progress: lesson count changed (4 -> 8), so any
  //     existing lessons_completed count for this chapter is now
  //     meaningless against the new content. ---
  ...
  if (staleProgress && staleProgress.length > 0) {
    console.log(`\n⚠️  Reseteando progreso previo de ${staleProgress.length} usuario(s) en este capítulo (el contenido cambió de 4 a 8 lecciones, el conteo anterior ya no es válido)...`);
  ...
  console.log('\n' + (lessonsOk && pretestOk && posttestOk ? '✅ CAPÍTULO 1 INSERTADO EXITOSAMENTE' : '⚠️  CAPÍTULO 1 INSERTADO CON DISCREPANCIAS'));
  ...
  console.log('   Flujo: Pre-test → 8 lecciones → Post-test\n');
